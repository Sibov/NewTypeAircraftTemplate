...
namespace CustomAircraftTemplateF35
{
    ...
    class F35_VTR_LoadScenariosPatch
    {
        public static List<VTScenarioInfo> Helper(string parentDirectory, bool checkModified, bool enforceDirectoryName, bool decodeWSScenarios)
        {
            ...
            while (true)
            {
                ...
                if (checkModified)
                {
                    Debug.Log("F35 - Skipped " + skippedCount + " scenarios (not modified).");
                }
    ...
    class F35_GetPlayerVehiclesCheck
    {
        public static bool Prefix(VTResources __instance, string vehicleName, ref PlayerVehicle __result)
        {
            ...
            {
                Debug.Log("F35 GPV - pvname = " + playerVehicle.vehicleName + ", " + vehicleName);
    ...
    public static class F35_MMPatch
    {
        public static bool Prefix(MeasurementManager __instance)
        {
            ...
            Debug.Log("F35 MM Patch 1.0");
            Traverse traverse = Traverse.Create(__instance);
            Debug.Log("F35 MM Patch 1.1");
            if (PilotSaveManager.current != null && PilotSaveManager.currentVehicle != null)
            {
                Debug.Log("F35 MM Patch 1.2");
    ...
    public static class CSIPatchUtils
    {
        ...
        public static IEnumerator BetterSetupCampaignScreenRoutine(CampaignSelectorUI instance)
        {
            ...
            Debug.Log("F35 BSCSCR Patch 1.2");
            ...
            Debug.Log("F35 BSCSCR Patch 1.3");
            if (_campaignsParent)
            {
                Debug.Log("F35 BSCSCR Patch 1.4");
            ...
            Debug.Log("F35 BSCSCR Patch 1.5");
            ...
            Debug.Log("F35 BSCSCR Patch 1.6");
            ...
            if (PilotSaveManager.currentVehicle.vehicleName == Main.customAircraftPV.vehicleName)
            {
                Debug.Log("F35 BSCSCR Patch 1.6.1");
                _aircraft.Add("F/A-26B");


            }
            Debug.Log("F35 BSCSCR Patch 1.7");
            foreach (var vName in _aircraft)
            {
                Debug.Log("F35 BSCSCR Patch 1.8: " + vName);
            ...
            /* ---- END EDIT ---- */
            sw.Stop();
            Debug.Log("F35 [CSIPatch] Time loading BuiltInCampaigns: " + sw.ElapsedMilliseconds);
            sw.Reset();
            sw.Start();
            Debug.Log("F35 BSCSCR Patch 1.11");

            foreach (var vName in _aircraft)
            {
                foreach (var campaign in PilotSaveManager.GetVehicle(vName).campaigns)
                {
                    Debug.Log("F35 BSCSCR Patch 1.12: " + campaign + ", " + vName);

                    if (campaign.isSteamworksStandalone)
                    {
                        Debug.Log("F35 BSCSCR Patch 1.13");

                        if (!SteamClient.IsValid) continue;
                        traverse.Field("swStandaloneCampaign").SetValue(campaign);
                        Debug.Log("F35 BSCSCR Patch 1.14");
                    ...
                    else if (campaign.readyToPlay)
                    {
                        ...
                        Debug.Log("F35 BSCSCR Patch 1.18");

                    }
                }
            }
            sw.Stop();
            Debug.Log("F35 [CSIPatch] Time loading vehicle campaigns: " + sw.ElapsedMilliseconds.ToString());
            sw.Reset();
            sw.Start();
            VTResources.GetCustomCampaigns();
            sw.Stop();
            Debug.Log("F35 [CSIPatch] Time loading custom campaigns list: " + sw.ElapsedMilliseconds.ToString());
            sw.Reset();
            sw.Start();
            ...
            foreach (var vName in _aircraft)
            {
                Debug.Log("F35 BSCSCR Patch 1.19: " + vName);
            ...
            sw.Stop();
            Debug.Log("F35 [CSIPatch] Time converting custom campaigns ToIngameCampaigns: " + sw.ElapsedMilliseconds);
            sw.Reset();
            int num;
            Debug.Log("F35 BSCSCR Patch 1.22");
            for (var i = 0; i < _campaigns.Count; i = num + 1)
            {
                Debug.Log("F35 BSCSCR Patch 1.23");
            ...
            Debug.Log("F35 BSCSCR Patch 1.28");
            ...
            if (setupList == null)
            {
             //   Debug.Log("F35 BSCSCR Patch 1.28.1");
            }
    ...
    class F35_CSIPatch_FinallyOpenCampaignSelector
    {
        ...
        static IEnumerator CSI_FinallyOpenCampaignSelectorRoutine(Action onOpenedSelector)
        {
            ...
            Debug.Log("F35 CSI Patch 1.2.1.3");
            if (String1 == null) { yield break; }
            Debug.Log("F35 CSI Patch 1.2.1" + PilotSaveManager.currentCampaign.campaignName);
            if (!PilotSaveManager.currentCampaign) yield break;
            Debug.Log("F35 CSI Patch 1.4");
            ...
            Debug.Log("F35 CSI Patch 1.5");
            if (num >= 0)
            {
                Debug.Log("F35 CSI Patch 1.6");
            ...
            else if ((bool)traverse.Field("openedWorkshopCampaign").GetValue()) //CampaignSelectorUI.openedWorkshopCampaign
            {
                ...
                {
                    Debug.Log("F35 CSI Patch 1.14");
                    while (enumerator.MoveNext())
                    {
                        Debug.Log("F35 CSI Patch 1.15");
                        ...
                        Debug.Log("F35 CSI Patch 1.16");
                        var _viewingCampaign = (Campaign)traverse.Field("viewingCampaign").GetValue();
                        while (_viewingCampaign == null)
                        {
                            Debug.Log("F35 CSI Patch 1.17");
                            _viewingCampaign = (Campaign)traverse.Field("viewingCampaign").GetValue();
                            yield return null;
                        }
                        if (lastCSave == null)
    ...
    class F35_CSIPatch_StartWorkshopCampaign
    {
        ...
        static IEnumerator CSI_StartWorkshopCampaignRoutine(string campaignID)
        {
            ...
            Debug.Log("F35 CSI Patch 2.2");
            var campaign = VTResources.GetSteamWorkshopCampaign(campaignID);
            if (campaign == null)
            {
                Debug.Log("F35 CSI Patch 2.3");
                Debug.Log("F35 Missing campaign in CSI_StartWorkshopCampaignRoutine");
            }
    ...
    class F35_CSIPatch_StartWorkshopMission
    {
        static CampaignSelectorUI instance;
    ...
    class F35_PlayerVehicleListPatch
    {
    ...
    class F35_Patch_VTEdUnitOptionsWindow_SetupOptionsList
    {
        public static void Postfix(VTEdUnitOptionsWindow __instance)
        {
            Debug.Log("F35 VTESOLPatch 1.0");

            var t = Traverse.Create(__instance);
            Debug.Log("F35 VTESOLPatch 1.1");
