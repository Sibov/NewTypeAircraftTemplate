...
namespace CustomAircraftTemplateF35
{
    static class FileLoader
    {
        ...
        {
            Debug.Log("F35 AssetBundleLoader: Attempting to load AssetBundle...");
            AssetBundle bundle = null;
            try
            {
                bundle = AssetBundle.LoadFromFile(path);
                Debug.Log("F35 AssetBundleLoader: Success.");
                return (AssetBundle)bundle;
            }
            catch (Exception e)
            {
                Debug.Log("F35 AssetBundleLoader: Couldn't load AssetBundle from path: '" + path + "'. Exception details: e: " + e.Message);
                return null;
            }
        ...
        {
            Debug.Log("F35 AssetBundleLoader: Attempting to retrieve: '" + name + "' as type: 'GameObject'" + " from " + bundleLoad);
            try
            {
            ...
            {
                Debug.Log("F35 AssetBundleLoader: Couldn't retrieve GameObject from AssetBundle.");
                return null;
            }
        ...
        {
            Debug.Log("F35 AssetBundleLoader: Attempting to retrieve: '" + name + "' as type: 'Scriptable'" + " from " + bundleLoad);
            try
            {
